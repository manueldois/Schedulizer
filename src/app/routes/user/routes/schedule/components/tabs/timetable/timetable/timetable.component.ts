  ...
  onDropToTimetable(event) {
    ...
    switch (source.tagName.toLowerCase()) {
      case 'app-available-task':
        console.log('Dropped available task to timetable: ', { task_id: source_id , volunteer_id: target_id, drop_relative_x })
        break;

      case 'app-scheduled-task':
        console.log('Dropped scheduled task to timetable: ', { task_id: source_id, volunteer_id: target_id, drop_relative_x })
        break;
